...
#### Custom API endpoints: `publicClients`
...
The `contracts.filter` property now supports multiple log filters, and requires argument values. [Read more](/docs/contracts-and-networks#event-filter).
...
:::info
Each deployment/instance of a Ponder app must have it's own schema, with some exceptions for `ponder dev` and crash recovery. [Read more](/docs/getting-started/database#database-schema).
:::
