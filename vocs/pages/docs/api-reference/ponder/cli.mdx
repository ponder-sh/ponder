...
# Ponder CLI reference [Commands for local dev, production, and administration]

The CLI (provided by the `"ponder"{:ts}` package) is the entrypoint for your project.
...
<Callout type="info">
  When using `ponder dev`, codegen runs automatically when the dev server
  detects a change in `ponder.schema.ts`.
</Callout>
