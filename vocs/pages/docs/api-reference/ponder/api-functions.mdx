...
API functions are built with [Hono](https://hono.dev/), a fast and lightweight routing framework. Hono supports all standard HTTP methods and an intuitive middleware API. Visit the Hono [documentation](https://hono.dev/) for more details.
...
The `src/api/index.ts` file _must_ **default export** a Hono app instance. You can use other files in `src/api/` to organize your code, just be sure to export your final Hono app correctly.

```ts [src/api/index.ts] {1,3,9}
