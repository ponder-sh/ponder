...
The factory pattern makes it possible to index a dynamic list of contracts by address. You can think of the `factory()` function as returning a list of addresses that updates whenever a log is found that matches the configuration.
...
The indexing functions you register for a contract (or account) that uses `factory()` receive events for all contracts (or accounts) matched by the factory configuration.
...
To index the factory contract itself, add a new entry to the `contracts` object. This entry works like a normal contract with a single address.
...
Sometimes, multiple factory contracts have the same factory event signature and create the same kind of child contract.

In these cases, you can pass a list of factory contract addresses to the `factory()` function, and the list of child addresses will be fetched from all factories and merged into a single list.
...
The factory pattern simply collects a list of addresses from a specific event log emitted by any contract. The contract does not have to be a "factory" in the Solidity/EVM sense (it doesn't need to create other contracts).
...
The sync engine doesn't support factory patterns that are nested beyond a single layer.
