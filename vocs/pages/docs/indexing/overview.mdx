...
To register an indexing function, use `ponder.on(...)`. The first argument is the event name as configured in `ponder.config.ts`, and the second argument is the function / callback that the indexing engine calls to process the event.
...
For each chain, the indexing engine calls indexing functions according to EVM execution order (block number, transaction index, log index). The ordering guarantee across multiple chains is more complex. [Read more](/docs/api-reference/ponder/config#event-ordering).
