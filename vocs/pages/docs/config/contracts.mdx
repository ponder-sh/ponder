...
# Contracts [Configure contracts in ponder.config.ts]

To add a new contract to your app, use the `contracts` field in `ponder.config.ts`. For each contract you add, the indexing engine will fetch raw blockchain data, decode it, and pass it to the indexing functions you register.

This guide explains how each contract configuration option works, and suggests patterns for common use cases. Visit the config [reference](/docs/api-reference/config) for complete API documentation.

## Contract name

Each contract must have a unique name, provided as a key to the `contracts` object.
...
```ts [ponder.config.ts]
import { createConfig } from "ponder";
import { BlitmapAbi } from "./abis/Blitmap"; // [!code focus]
...
Use the [`mergeAbis`](/docs/utilities/merge-abis) utility function to combine multiple ABIs into one. This function removes duplicate ABI items and maintains strict types.
...
The `startBlock` option specifies the block number to begin indexing from. The default is `0` – it's a good idea to set `startBlock` to the contract deployment block number to avoid wasteful RPC requests.
...
<Callout type="info">
  Visit the [call traces](/docs/guides/traces) guide for more information.
</Callout>
...
Use the `includeTransactionReceipts` option to fetch transaction receipts for each event, which makes the `event.transactionReceipt` property available in all indexing functions for the contract.
