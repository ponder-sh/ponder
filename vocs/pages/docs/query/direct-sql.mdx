...
:::warning
Direct SQL is more complex than querying over HTTP with GraphQL or the SQL client. If either of those options are sufficient for your use case, consider using them instead.
:::
...
Ponder uses **database schemas** to isolate deployments. [Read more](/docs/production/overview) in the self-hosting guide.

Direct SQL queries should target the database schema corresponding to a specific deployment. Or, if your project uses the [views pattern](/docs/production/overview) (**recommended**), queries should target the views schema.
...
This function mutates a Drizzle schema such that any queries built using the schema will be qualified with the target database schema. [Read more](https://orm.drizzle.team/docs/schemas) in the Drizzle documentation.

:::code-group

```ts [Drizzle]
import { drizzle } from "drizzle-orm/node-postgres";
import * as schema_ from "../../ponder/ponder.schema"; // [!code focus]

const schema = setDatabaseSchema(schema_, "target_schema"); // [!code focus]
...
### Join Ponder data with other data

It's often useful to join Ponder data with other tables in your database.
