...
<Callout type="info">
  Relations **do not** create foreign key constraints and won't stop
  you from inserting rows that violate referential integrity. They exist only to enrich the
  GraphQL API and Drizzle Query API (`findMany` and `findFirst`).
</Callout>
