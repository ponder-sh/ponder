...
// 1. Create database
await db.execute(sql.raw(`CREATE DATABASE "benchmark_${APP_ID}"`));

db = drizzle(`${process.env.DATABASE_URL!}/benchmark_${APP_ID}`, {
  casing: "snake_case",
});
...
process.env.DATABASE_URL = `${process.env.DATABASE_URL!}/benchmark_${APP_ID}`;
