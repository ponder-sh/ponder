...
The [`p.json(){:ts}` column type](/docs/schema#json-type) uses the `JSONB{:sql}` data type in PGlite and Postgres, which [does not support](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/BigInt#use_within_json) `BigInt` values.
