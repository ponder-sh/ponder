...
To enable code reuse and maintain type safety for advanced use cases, Ponder offers several utility types.

## Indexing functions types

The `"@/generated"{:ts}` module exports utility types that are useful for creating reusable helper functions in your indexing files. Like the `ponder` object, these types are aware of the values you specify in your `ponder.config.ts` and `ponder.schema.ts` files.
