...
```ts 
import { type SQL, asc } from "@ponder/client";
import { usePonderClient, getQueryKey } from "@ponder/react";
import { useInfiniteQuery } from "@tanstack/react-query";

export const useInfinitePersonsQuery = (params: {
  where: SQL | undefined;
  limit: number;
}) => {
  ...
  return useInfiniteQuery({
    queryKey: ["pools", ...getQueryKey(params.where), params.limit],
    queryFn: ({ pageParam }) =>
      client.db
        .select()
        .from(schema.person)
...
https://github.com/whetstoneresearch/doppler-sdk/blob/main/packages/doppler-v3-indexer/ponder.schema.ts

```ts
export const usePools = (params: {
  limit = 10,
}) => {
  return usePonderQuery({
    queryFn: (db) => db.query.pool.findMany({
