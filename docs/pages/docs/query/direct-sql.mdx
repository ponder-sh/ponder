...
3. **Redeployments are nuanced**. Apps that rely on direct SQL must take care during schema migrations. For more details, see the [zero downtime deployments](/docs/production/zero-downtime#zero-downtime-with-direct-sql) guide.
