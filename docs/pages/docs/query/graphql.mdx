...
| `{column}_contains_nocase` | String primitives             | case-insensitively **contains** the substring |
| `{column}_not_contains_nocase` | String primitives         | case-insensitively **does not contain** the substring |
...
| `{column}_starts_with_nocase` | String primitives          | case-insensitively **starts with** the substring |
| `{column}_not_starts_with_nocase` | String primitives      | case-insensitively **does not start with** the substring |
...
| `{column}_ends_with_nocase` | String primitives            | case-insensitively **ends with** the substring |
| `{column}_not_ends_with_nocase` | String primitives        | case-insensitively **does not end with** the substring |
...
Get all `person` records with a `name` that starts with `"p"` regardless of case.

:::code-group

```graphql [Query]
query {
  persons(where: { name_starts_with_nocase: "p" }) {
    name
    age
  }
}
...
:::warning
The SQL query that backs `totalCount` can be slow. To avoid performance issues, consider including `totalCount` for the first page, then exclude it for subsequent pages.
:::
...
:::info
  Note that plural query fields for views (`onchainView`) do not support cursor pagination. [Read more](/docs/schema/views).
:::
