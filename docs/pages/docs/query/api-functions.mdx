...
- `/health`: Returns a `200` status code if the app is running. [Read more](/docs/production/deploy) about healthchecks.
- `/ready`: Returns a `200` status code after the app has completed the historical backfill and is available to serve traffic.[Read more](/docs/production/deploy) about heatlthchecks.
