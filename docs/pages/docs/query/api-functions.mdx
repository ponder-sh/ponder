...
The API function context includes a ready-to-use Drizzle database client exported from `ponder:api`. To query the database, import table objects from `ponder.schema.ts` or `ponder:schema` and pass them to `db.select()` or use relational queries.
