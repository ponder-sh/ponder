---
title: "Contracts & chains"
description: "Learn how to add contracts and chains in ponder.config.ts"
---
...
```ts filename="ponder.config.ts" {1,13}
...
export default createConfig({
  chains: {
    goerli: { id: 5, rpcUrl: process.env.PONDER_RPC_URL_5 },
  },
  contracts: {
...
If the contract is only deployed to one chain, just pass the chain name as a string to the `chain` field.

```ts filename="ponder.config.ts" {7,15}
import { createConfig } from "ponder";
...
If you'd like to index the same contract (having the same ABI) across multiple chains, pass an object to the `chain` field containing chain-specific options.

```ts filename="ponder.config.ts" {13-22}
import { createConfig } from "ponder";
...
export default createConfig({
  chains: {
    mainnet: { id: 1, rpcUrl: process.env.PONDER_RPC_URL_1 },
    base: { id: 8453, rpcUrl: process.env.PONDER_RPC_URL_8453 },
  },
  contracts: {
    UniswapV3Factory: {
...
The `event` and `context` objects are still strictly typed according to the configuration you provide. The `context.chain` object contains information about which chain the current event is from.
...
Chain-specific configuration uses an override pattern. Any options defined at the top level are the default, and the chain-specific objects override those defaults. All fields other than `abi` can be specified on a per-chain basis, including `address`, `filter`, `startBlock`, and `endBlock`.
...
export default createConfig({
  chains: {
    mainnet: { id: 1, rpcUrl: process.env.PONDER_RPC_URL_1 },
    optimism: { id: 10, rpcUrl: process.env.PONDER_RPC_URL_10 },
    base: { id: 8453, rpcUrl: process.env.PONDER_RPC_URL_8453 },
  },
  contracts: {
    UniswapV3Factory: {
      ...
      address: "0x1F98431c8aD98523631AE4a59f267346ea31F984",
      chain: {
        // No chain-specific address provided for mainnet and Optimism.
        ...
        // The chain-specific address will be used ("0x33128a8fC17869897dcE68Ed026d694621f6FDfD").
        base: {
          address: "0x33128a8fC17869897dcE68Ed026d694621f6FDfD",
          startBlock: 1371680,
        },
...
On the other hand, the ERC-4337 Entry Point contract is deployed to the same address on all chains, so you could define the `address` field at the top level.
...
```ts filename="ponder.config.ts" {13}
import { createConfig } from "ponder";
...
```ts filename="ponder.config.ts" {13-18}
import { createConfig } from "ponder";
...
The factory `address` field also accepts a list of factory contract addresses. Use this option if there are multiple factory contracts on the same chain that have the same ABI, factory event signature, and create the same kind of child contract.
...
```ts filename="ponder.config.ts" {14-15}
import { createConfig } from "ponder";
