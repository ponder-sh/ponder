...
#### Removed `networks` in `ponder.config.ts`

Introduced a new `chains` property in `ponder.config.ts`. This replaces the `networks` property.

- `networks` is renamed to `chains`
- `chainId` is renamed to `id`

The `rpc` property replaces `transport`. `rpc` accepts type `string | string[] | viem.Transport`. When multiple RPC URLS are provided, Ponder load balances across them.

<div className="code-columns">

```ts filename="ponder.config.ts (<=0.10)"
...
```ts filename="ponder.config.ts (0.11)" {4-9, 12}
import { createConfig } from "ponder";
...
#### Removed `context.network`

`context.network` is removed. Use `context.chain` instead.

#### Default `multichain` ordering

Multichain ordering is now the default. Read more about ordering [here](/docs/api-reference/config#ordering).

#### `publicClients`

The [`publicClients`](/docs/api-reference/api-functions#send-rpc-requests) object is now keyed by chain name rather than chain ID.

#### `/status` response

The response type for the `/status` endpoint has been updated.
...
#### `_ponder_status` table

The `_ponder_status` table was replaced with the `_ponder_checkpoint` table.

### New features

#### `ponder db create-views` and `ponder start --views-schema <schema>`
...
#### API function file is required

The API function file `src/api/index.ts` is now required. The GraphQL API is no longer served by default.
...
This release makes the API layer less opinionated. Just default export a normal Hono `App` object from the `src/api/index.ts` file, and Ponder will serve it.
...
The `@ponder/client` package provides a new experience for querying a Ponder app over HTTP. It's an SQL-based alternative to the GraphQL API. [Read more](/docs/query/client).

#### `@ponder/react`

The `@ponder/react` package uses Tanstack Query to

#### `publicClients` in API functions

API function files now have access to a new `"ponder:api"` virtual module. This module contains the `db` object and a new `publicClients` object, which contains a Viem [Public Client](https://viem.sh/docs/clients/public) for each network. These clients use the transports defined in `ponder.config.ts`.
