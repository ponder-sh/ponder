...
#### Live indexing uses in-memory database

Live indexing now uses the same in-memory database as backfill indexing.

#### Deprecated Subgraph and Etherscan in `create-ponder`

Subgraph and Etherscan templates are deprecated in `create-ponder`.

### New features

#### Bun support

Ponder now supports the [Bun](https://bun.sh) runtime. Update the scripts in `package.json` to use Bun.
...
Visit the [documentation](/docs/guides/bun) for more details.
