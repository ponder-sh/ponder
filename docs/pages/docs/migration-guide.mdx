...
1. Import `onchainTable` and database helper functions from `@ponder/core/db`
2. Declare tables with `onchainTable`, 
3. Export tables from `ponder.schema.ts`
4. Specify primary key columns with `.primaryKey()`
5. Specify not null columns with `.notNull()`
...
```
6. `evmHex` and `evmBigint` are special columns exported from `@ponder/core/db`
```ts filename="ponder.schema.ts"
...
1. Run `pnpm codegen` to update types
2. Import tables from `ponder.schema.ts`
```ts filename="src/index.ts"
import { account } from "../ponder.schema";
```
3. Replace `.findUnique()` with `.find()`
...
5. Replace `.createMany()` with `.insert()`
```diff filename="src/index.ts"
...
9. Use low-level SQL client for advanced queries

`context.db.sql` provides a drizzle interface which can be used to make arbitrary sql queries.
```ts filename="src/index.ts"
...
1. Define offchain tables with `offchainTable()` in `ponder.schema.ts`
```ts filename="ponder.schema.ts"
...
```
2. Use `offchainSchema()` to create offchain tables in a specific schema
```ts filename="ponder.schema.ts"
...
```
3. Add `drizzle-kit` as a dependency
4. Add migration generation script to `package.json`
...
```
5. Run `pnpm generate` to generate migrations for offchain tables
6. Run `pnpm dev` or `pnpm start` to automatically apply migrations
