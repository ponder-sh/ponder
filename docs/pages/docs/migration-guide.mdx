...
API functions are required and Ponder will fail to start if `src/api/index.ts` does not exist.
...
The `ponder.get()` function was removed. Ponder now requires the explicit use of **Hono with a default export**.
...
The `db` object is now injected into the API function context. `c.db` is removed.
