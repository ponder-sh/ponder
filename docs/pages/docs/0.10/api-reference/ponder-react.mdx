...
[SQL Client](/docs/0.10/query/sql-client) page.
:::

The `@ponder/react` package provides React hooks for subscribing to live updates from your database.

This package uses [`@ponder/client`](/docs/0.10/api-reference/ponder-client) to execute SQL queries over HTTP, and integrates with [TanStack Query](https://tanstack.com/query) for async state management.
...
- Uses [`client.live`](/docs/0.10/api-reference/ponder-client#clientlive) to automatically refetch data when new blocks are indexed
...
function IndexingStatus() {
  ...
  return (
    <div>
      {Object.entries(data).map(([network, status]) => (
        <div key={network}>
          <h3>{network}</h3>
...
Returns a normal TanStack `useQuery` result. [Read more](https://tanstack.com/query/latest/docs/framework/react/reference/useQuery) in the TanStack documentation.
