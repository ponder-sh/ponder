---
title: "Blocks"
description: "Learn how to add block event sources to a Ponder app"
---
...
To run indexing functions on a schedule, add a **block event** source to your config. Block events are great for "cron" workloads, because they run at a consistent frequency regardless of contract activity or transactions.

This guide describes how to configure block event sources, and suggests patterns for common use cases. See also the `ponder.config.ts` [API reference](/docs/api-reference/config) page.

## Name

Every block event source must have a name, provided as a key to the `blocks` object. The name must be unique across `blocks` and `contracts`.
...
The `interval` option specifies how often a block event should run. For example, a block event source with a start block of `100` and a interval of `10` will index blocks `100`, `110`, `120`, `130`, and so on.
...
Now, the indexing functions you write for `ChainlinkOracleUpdate:block` will process block events from both mainnet and Optimism.

The `context.network` object contains information about which network the current event is from.
