...
It's often easier to think about a time interval instead of a block interval. To convert between the two, divide the time interval by the chain block time.
...
```ts filename="ponder.config.ts" {11-12}
import { createConfig } from "ponder";
