...
Add the chain, address, and start block to the `accounts` field in `ponder.config.ts`.
...
The `chain` option for accounts works the same way as it does for contracts. [Read more](/docs/contracts-and-chains#chain).

## Address

The `address` option for accounts works the same way as it does for contracts. You can provide a single address, a list of addresses, or an address factory. You can also specify chain-specific overrides. [Read more](/docs/contracts-and-chains#address).

## Block range

The `startBlock` and `endBlock` options for accounts work the same way as it does for contracts. [Read more](/docs/contracts-and-chains#block-range).
