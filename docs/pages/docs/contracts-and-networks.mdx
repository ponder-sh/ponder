...
<Callout type="info">
  If you're indexing a popular contract but only need an infrequent event (e.g.
  `OwnershipTransferred`), you can use the `filter` option alongside `address`
  or `factory` to sync only those event(s). By default, the sync engine fetches
  all events emitted by the contracts you specify.
</Callout>
