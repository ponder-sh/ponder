...
<Callout type="default">
  ...
  option](/docs/contracts-and-chains#call-traces) must be enabled to use call
  trace events.
</Callout>
...
The `context.chain` object includes information about the chain that the current event is from. The object is strictly typed according to the chains you defined in your config.

```ts filename="src/index.ts"
ponder.on("UniswapV3Factory:Ownership", async ({ event, context }) => {
  context.chain;
  //      ^? { name: "mainnet", id: 1 } | { name: "base", i: 8453 }
