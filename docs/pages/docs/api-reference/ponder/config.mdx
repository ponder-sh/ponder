...
| field                                    |          type           |                                                                                       |
| :--------------------------------------- | :---------------------: | :------------------------------------------------------------------------------------ |
| **max**                                  |        `number`         | **Default: `30`**. Maximum number of clients in the pool.                            |
| **ssl**                                  | `boolean \| SSLConfig`  | **Default: `false`**. Enable SSL, or provide a custom SSL configuration.             |
| **idleInTransactionSessionTimeout**  |        `number`         | **Default: `3600000`**. Timeout for idle transactions in milliseconds (1 hour).     |

```ts [ponder.config.ts] {4-11}
import { createConfig } from "ponder";
...
export default createConfig({
  database: {
    ...
    poolConfig: {
      max: 100,
      ssl: true,
      idleInTransactionSessionTimeout: 7200000, // 2 hours
    },
  },
  // ...
});
