...
| field        |             type                                       |                              |
| :----------- | :----------------------------------------------------: | :--------------------------- |
| **ordering** | `"omnichain" \| "multichain" \| "isolated"` | **Default:** `"multichain"`. |
...
- `"isolated"` is a subset of `"multichain"` with stricter rules on cross-chain database access. **`"isolated"` uses a thread per chain and allows one chain to fail without affecting the others.**

:::note
  **Isolated ordering (experimental):** When using `ordering: "isolated"`, all tables in your schema must include a `chainId` column that is part of the primary key. This ensures complete isolation between chains and prevents any cross-chain data access.
:::
