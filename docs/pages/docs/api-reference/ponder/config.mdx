...
| **eth_getLogsRangeLimit** |  `number`                          | **Default: `undefined`**. Maximum block range for `eth_getLogs` requests. If undefined, Ponder will attempt to determine the range limit automatically based on error messages. |
