...
The `ponder.config.ts` file contains contract names, addresses, and ABIs; chain information like chain IDs and RPC URLs; database configuration; and general options.
...
{/* prettier-ignore */}
```ts filename="ponder.config.ts" {1,3}
...
| field        |             type              |                                                      |
| :----------- | :---------------------------: | :--------------------------------------------------- |
| **ordering** | `"omnichain" \| "multichain"` | **Default:** `"multichain"`. Event ordering strategy.|
...
The `chains` field is an object where each key is a chain name containing that chain's configuration. Chains are Ethereum-based blockchains like Ethereum mainnet, Goerli, or Foundry's local Anvil node.
...
| field                    |          type                              |                                                                                                                            |
| :----------------------- | :----------------------------------------: | :------------------------------------------------------------------------------------------------------------------------- |
| **name**                 |        `string`                            | A unique name for the blockchain. Must be unique across all chains. _Provided as an object property name._                 |
| **id**                   |        `number`                            | The [chain ID](https://chainlist.org) for the chain.                                                                       |
| **rpc**                  | `string` \| `string[]` \| `viem.Transport` | An RPC URL or Viem `http`, `webSocket`, or `fallback` [Transport](https://viem.sh/docs/clients/transports/http.html).      |
| **pollingInterval**      | `number \| undefined`                      | **Default: `1_000`**. Frequency (in ms) used when polling for new events on this chain.                                    |
...
```ts filename="ponder.config.ts" {6-11,15}
import { createConfig } from "ponder";
...
<Callout>
  ...
  the [contracts & chains](/docs/contracts-and-chains) guide.
</Callout>
...
| **chain**                      |                       `string`                       | The name of the chain this contract is deployed to. References the `chains` field. Also supports [multiple chains](/docs/contracts-and-chains#multiple-chains).                                                          |
...
[Read more](/docs/contracts-and-chains#event-filter) about event filters.
...
[Read more](/docs/contracts-and-chains#factory-contracts) about factory patterns.
