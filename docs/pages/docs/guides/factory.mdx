...
In these cases, to save indexing time you can specify `startBlock` and `endBlock` of the factory separately. The `startBlock` option specifies the block number to begin collecting factory childs from. The `endBlock` option specifies the block number to stop collecting factory childs at. Both options default to contract's `startBlock` and `endBlock` respectively. 
