...
This guide uses `drizzle-orm`, however, the same principles apply to any method of interacting with a Postgres database.

:::steps

### Repo structure and `drizzle-orm` version

In either repository structure, it is important to make sure the `drizzle-orm` version used is the same as the one used by `ponder`. Currently, that version is **`0.41.0`**.
...
This file will be used to generate migrations for offchain data.
