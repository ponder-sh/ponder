...
Ponder does its best to automatically determine the maximum block range for JSON-RPC requests with the `eth_getLogs` method. When this comes up short, you can set a manual limit with `eth_getLogsBlockRange` option. 
...
export default createConfig({
  chains: {
    mainnet: {
      id: 1,
      rpc: "https://eth-mainnet.g.alchemy.com/v2/...",
      eth_getLogsBlockRange: 1000, // [!code focus]
    },
  },
});
```
