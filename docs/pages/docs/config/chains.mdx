...
If you provide multiple endpoints, Ponder intelligently balances requests across them, and uses fallback logic to improve reliability. 

The indexing engine dynamically manages RPC request rates based on the provider's rate limits and network conditions for both multiple and singular endpoints. 
