...
This object contains CRUD objects for the tables defined in `schema.graphql`, and a read-only contract object for each contract specified in `ponder.config.ts`.
