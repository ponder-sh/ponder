---
description: "API reference for the ponder.ts file"
---
...
The `ponder.ts` config file contains smart contract addresses, paths to ABIs, RPC URLs, database config, and plugin config.

Some things to know:

- `ponder.ts` must provide a **named export called `config`**, not a default export.
- By default, `ponder {dev/start/codegen}` looks for `ponder.ts` in the current working directory, but you can specify a different path with the `--config-file` option to `ponder dev` or `ponder start`.
- The `config` variable exported by `ponder.ts` can be an object, a function, or a function returning a Promise (you can do async stuff to determine your config).
...
Sources are smart contracts deployed to a network. Ponder will fetch all the event logs emitted by each source defined here.

| field          |         type          |                                                                                                                                                                  |
| :------------- | :-------------------: | :--------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **name**       |       `string`        | A unique name for the smart contract. **Must be unique across all sources.**                                                                                     |
