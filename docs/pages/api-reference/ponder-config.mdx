...
The `ponder.config.ts` config file contains smart contract addresses, paths to ABIs, RPC URLs, and database config.
...
| **startBlock** | `number \| undefined`  | **Default: `0`**. The block number to start handling events for this contract. Usually the contract deployment block number. **Default: `0`**                                                                                                                                     |
...
| **blockLimit** | `number \| undefined`  | The maximum block range that Ponder will use when calling `eth_getLogs`. If not provided, Ponder uses a sane default based on the RPC provider and chain ID for this contract.                                                                                                    |
| **isIndexed**  | `boolean \| undefined` | **Default: `true`**. Specifies if Ponder should process events for this contract. If `false`, Ponder will not fetch events for this contract, but it will still be available on the event handler `context.contracts`.                                                            |
