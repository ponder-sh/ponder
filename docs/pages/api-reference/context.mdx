...
The `context` object exposes the entities defined in `schema.graphql` and the contracts defined in `ponder.ts`.

```ts filename="Context.d.ts"
interface Context {
  // Keyed by entity type names from schema.graphql
  entities: Record<string, EntityModel>;
  // Keyed by source names from ponder.ts
...
`context.contracts` contains `ethers.Contract` objects for each source defined in your `ponder.ts`. You can use these to read data directly from the smart contract using RPC calls.
