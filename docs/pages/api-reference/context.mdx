...
interface Context {
  ...
  // Keyed by source names from ponder.config.ts
...
`context.contracts` contains `ethers.Contract` objects for each source defined in your `ponder.config.ts`. You can use these to read data directly from the smart contract using RPC calls.
