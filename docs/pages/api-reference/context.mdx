...
The `context` object exposes the entities defined in `schema.graphql` and the contracts defined in `ponder.config.js`.

```ts
interface Context {
  // Keyed by entity type names from schema.graphql
  entities: Record<string, EntityModel>;
  // Keyed by contract names from ponder.config.js
...
`context.contracts` contains `ethers.Contract` objects for each contract defined in your `ponder.config.js`. You can use these to read data directly from the smart contract using RPC calls.
