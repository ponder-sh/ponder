...
It's occasionally useful to merge ABIs, like when defining a proxy/upgradable contract that has gone through multiple implementation contracts. The `mergeAbis` function exported by `@ponder/core` safely removes duplicate ABI items and maintains strict types.
