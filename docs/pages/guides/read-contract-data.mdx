...
Ponder natively supports this pattern by injecting a custom [Viem Client](https://viem.sh/docs/clients/intro.html) into the indexing function context. This modified client automatically caches RPC requests and supports several RPC methods including `eth_call`, `multicall`, `eth_getStorageAt`, and `eth_getBytecode`.
...
To call a factory contract child from an indexing function for a _different_ contract, use your application logic to determine the correct address to enter. For example, the address might come from `event.params`.
...
The `context.client` object is a custom [Viem Client](https://viem.sh/docs/clients/intro.html).
...
ponder.on("ZorbNft:Transfer", async ({ event, context }) => {
  ...
  await context.models.Zorb.update({
    id: event.params.tokenId,
    data: {
      ownerId: event.params.to,
    },
