...
Contracts injected this way are ready-to-go using the address and network specified in `ponder.config.ts`. (You don't need to call `Contract.bind(address)`. The contracts are `ReadOnlyContract` objects that are already connected to a `viem` client`.
