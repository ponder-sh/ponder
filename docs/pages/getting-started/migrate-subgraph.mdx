...
The `create-ponder` package makes it easy to migrate a subgraph to Ponder.
...
```
  ...
  <Tab>
```bash
npm init ponder@latest -- --from-subgraph ...
```
</Tab>
</Tabs>
...
As you migrate your event handlers and start inserting Entities, open the GraphiQL interface at `http://localhost:42069/graphl` to explore your GraphQL API locally. Any changes you make to your `schema.graphql` file will be reflected here.

That's it! When you're ready to deploy to production.
