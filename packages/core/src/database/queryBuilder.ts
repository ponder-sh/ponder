...
>(
    ...
    for (let i = 0; i <= RETRY_COUNT; i++) {
      ...
      try {
        ...
        if (
          ...
          common.logger.warn({
            service: "database",
            msg: `Failed '${sql}' database query (id=${id})`,
            error,
          });
          throw error;
        }
        ...
        if (i === RETRY_COUNT) {
          common.logger.warn({
            service: "database",
            msg: `Failed '${sql}' database query after '${i + 1}' attempts (id=${id})`,
            error,
          });
          throw firstError;
        }
        ...
        common.logger.debug({
          service: "database",
          msg: `Failed '${sql}' database query, retrying after ${duration} milliseconds (id=${id})`,
          error,
        });
        await wait(duration);
      } finally {
  ...
  db._.session.transaction = async (...args) => {
    ...
    return wrap(() => transaction(...args), "begin");
  };
