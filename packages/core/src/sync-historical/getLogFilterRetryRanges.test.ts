...
test("getLogFilterRetryRanges handles Quicknode 'eth_getLogs and eth_newFilter are limited to a 10,000 blocks range' error", async () => {
  ...
  expect(retryRanges).toHaveLength(2);
  expect(retryRanges[0]).toMatchObject([numberToHex(1), numberToHex(10_000)]);
