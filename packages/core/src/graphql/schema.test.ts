...
import { GraphQLInputObjectType, execute, parse } from "graphql";
import { beforeEach, expect, test } from "vitest";
import { buildLoaderCache } from "./loader.js";
...
test("filter type has correct suffixes and types", () => {
  ...
  const filterType = graphqlSchema.getTypeMap()
    .TableFilter as GraphQLInputObjectType;

  const filterFields = filterType.toConfig().fields;

  const filterFieldsAsString = Object.entries(filterFields).reduce(
    (acc, [key, value]) => {
  ...
  expect(filterFieldsAsString).toMatchObject({
    AND: "[TableFilter]",
    OR: "[TableFilter]",
    id: "String",
    id_not: "String",
