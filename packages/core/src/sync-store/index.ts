...
const formatRawEvent = ({ filters, row }: { filters: Filter[]; row: any }) => {
  // Without this cast, the block_ and tx_ fields are all nullable
  // which makes this very annoying. Should probably add a runtime check
  // that those fields are indeed present before continuing here.
  row = row as NonNull<typeof row>;
...
export const createSyncStore = ({
  ...
  getEvents: async ({ filters, from, to, limit }) => {
    const query = getEventsQuery({ filters, from, to, limit, db });
    ...
    for (const row of rows) {
      events.push(formatRawEvent({ filters, row }));
    }
  ...
  getEventsStream: async function* ({
    filters,
    from,
    to,
    limit,
    ...
    const stream = await db.wrap(
      ...
      async () => {
        return query.stream(10000);
      },
    );
    for await (const row of stream) {
      yield formatRawEvent({ filters, row });
    }
  },
