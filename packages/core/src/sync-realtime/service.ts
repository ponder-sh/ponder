...
export class RealtimeSyncService extends Emittery<RealtimeSyncEvents> {
    ...
    try {
      [latestBlock, rpcChainId] = await Promise.all([
        ...
        this.network.requestQueue
          .request("realtime", { method: "eth_chainId" })
          .then(hexToNumber),
      ]);
    } catch (error_) {
      throw Error(
  ...
  start = async () => {
    ...
    const finalizedBlock = await this.network.requestQueue.request("realtime", {
  ...
  private blockTaskWorker = async ({
    ...
    if (
      ...
      if (!this.sources.some(sourceIsFactory)) {
        ...
        if (!doesBlockHaveLogFilterLogs) {
          ...
          const stopClock = startClock();
          logs = await this.network.requestQueue.request("realtime", {
            method: "eth_getLogs",
    ...
    while (canonicalBlock.number > (this.finalizedBlockNumber ?? -1)) {
      ...
      const stopClock = startClock();
      const parentBlock_ = await this.network.requestQueue.request("realtime", {
