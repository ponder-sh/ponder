    ...
    for (let i = 0; i <= RETRY_COUNT; i++) {
      try {
        ...
        if (shouldRetry(error) === false) {
          common.logger.warn({
            service: "sync",
            msg: `Failed '${request.method}' RPC request with non-retryable error: ${error}`,
          });
          throw error;
        }
        ...
        common.logger.debug({
          service: "sync",
          msg: `Failed '${request.method}' RPC request, retrying after ${duration} milliseconds. Error: ${error}`,
        });
        await wait(duration);
      }
    }
