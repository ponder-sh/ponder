...
export const createSync = async (args: CreateSyncParameters): Promise<Sync> => {
  ...
  // Note: I think this entire `createSync` function should take a network parameter
  let localData: Omit<Extract<RealtimeSyncEvent, { type: "block" }>, "type">[] =
    [];
        ...
        case "block":
          {
            ...
            const events = buildEvents(sources, event);

            updateStatus(events, checkpoint, true);
        ...
        case "finalize":
          {
            ...
            const finalizedData = localData.filter(
              (d) =>
