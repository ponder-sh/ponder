...
export const splitEvents = (
  ...
  for (const event of events) {
    if (hash === undefined || hash !== event.event.block.hash) {
      result.push({
        events: [],
        chainId: event.chain.chain.id,
        checkpoint: encodeCheckpoint({
          ...MAX_CHECKPOINT,
          blockTimestamp: event.event.block.timestamp,
          chainId: BigInt(event.chain.chain.id),
...
export const findChain = (
  ...
  return app.indexingBuild.find((ib) => ib.chain.chain.id === id)!.chain;
};
...
export const createSync = async (
  ...
  const perChainApps = getPerChainPonderApp(app);
  ...
  async function* getEvents() {
    ...
    const eventGenerators = perChainApps.map((perChainApp) => {
      const { syncProgress, historicalSync } = perChainSync.get(
        perChainApp.indexingBuild.chain,
      )!;
      ...
      async function* decodeEventGenerator(
        ...
        for await (const { events: rawEvents, checkpoint } of eventGenerator) {
          ...
          const decodedEvents = decodeEvents(perChainApp, { rawEvents });
          app.common.logger.debug({
            service: "app",
            msg: `Decoded ${decodedEvents.length} '${perChainApp.indexingBuild.chain.chain.name}' events`,
          });
      ...
      async function* sortCompletedAndPendingEvents(
        ...
        for await (const { events, checkpoint } of eventGenerator) {
          ...
          const _crashRecoveryCheckpoint = crashRecoveryCheckpoint?.find(
            ({ chainId }) =>
              chainId === perChainApp.indexingBuild.chain.chain.id,
          )?.checkpoint;
      ...
      const localEventGenerator = getLocalEventGenerator(perChainApp, {
        localSyncGenerator,
        from: getMultichainCheckpoint({
          tag: "start",
          chain: perChainApp.indexingBuild.chain,
        })!,
        to: min(
  ...
  const onRealtimeSyncEvent = (
    ...
    switch (event.type) {
      case "block": {
        const rawEvents = buildEvents(perChainApp, {
          blockData: {
        ...
        app.common.logger.debug({
          service: "sync",
          msg: `Extracted ${rawEvents.length} '${perChainApp.indexingBuild.chain.chain.name}' events for block ${hexToNumber(event.block.number)}`,
        });

        const decodedEvents = decodeEvents(perChainApp, { rawEvents });
        app.common.logger.debug({
          service: "sync",
          msg: `Decoded ${decodedEvents.length} '${perChainApp.indexingBuild.chain.chain.name}' events for block ${hexToNumber(event.block.number)}`,
        });
        ...
        if (app.preBuild.ordering === "multichain") {
          ...
          app.common.logger.debug({
            service: "sync",
            msg: `Sequenced ${readyEvents.length} '${perChainApp.indexingBuild.chain.chain.name}' events for block ${hexToNumber(event.block.number)}`,
          });
          ...
          onRealtimeEvent({
            ...
            if (event.endClock) {
              app.common.metrics.ponder_realtime_latency.observe(
                { chain: perChainApp.indexingBuild.chain.chain.name },
                event.endClock(),
              );
            }
          });
          ...
          if (event.endClock !== undefined) {
            latencyTimers.set(
              encodeCheckpoint(
                blockToCheckpoint(
                  event.block,
                  perChainApp.indexingBuild.chain.chain.id,
                  "up",
                ),
              ),
              event.endClock,
          ...
          if (to > from) {
            ...
            for (const [_chain, { realtimeSync }] of perChainSync) {
              const localBlock = realtimeSync.unfinalizedBlocks.findLast(
                (block) => {
                  const checkpoint = encodeCheckpoint(
                    blockToCheckpoint(block, _chain.chain.id, "up"),
                  );
              ...
              if (localBlock) {
                const checkpoint = encodeCheckpoint(
                  blockToCheckpoint(localBlock, _chain.chain.id, "up"),
                );

                checkpoints.push({ chainId: _chain.chain.id, checkpoint });
              }
            }
      ...
      case "finalize": {
        ...
        if (
          ...
          app.common.logger.warn({
            service: "sync",
            msg: `Finalized '${perChainApp.indexingBuild.chain.chain.name}' block has surpassed overall indexing checkpoint`,
          });
        }
      ...
      case "reorg": {
        ...
        app.common.logger.debug({
          service: "sync",
          msg: `Removed ${reorgedEvents} reorged '${perChainApp.indexingBuild.chain.chain.name}' events`,
        });
  ...
  await Promise.all(
    perChainApps.map(async (perChainApp) => {
      // Invalidate sync cache for devnet sources
      if (perChainApp.indexingBuild.chain.disableCache) {
        app.common.logger.warn({
          service: "sync",
          msg: `Deleting cache records for '${perChainApp.indexingBuild.chain.chain.name}'`,
        });
            ...
            .then((event) => {
              ...
              if (isSyncFinalized(syncProgress) && isSyncEnd(syncProgress)) {
                ...
                app.common.logger.info({
                  service: "sync",
                  msg: `Killing '${perChainApp.indexingBuild.chain.chain.name}' live indexing because the end block ${hexToNumber(syncProgress.end!.number)} has been finalized`,
                });
                realtimeSync.kill();
              }
            })
      ...
      perChainSync.set(perChainApp.indexingBuild.chain, {
        syncProgress,
        historicalSync,
        realtimeSync,
      });
  ...
  for (const perChainApp of perChainApps) {
    const _crashRecoveryCheckpoint = crashRecoveryCheckpoint?.find(
      ({ chainId }) => chainId === perChainApp.indexingBuild.chain.chain.id,
    )?.checkpoint;

    seconds[perChainApp.indexingBuild.chain.chain.name] = {
      start: Number(
      ...
      cached: Number(
        decodeCheckpoint(
          min(
            getOmnichainCheckpoint({ tag: "end" }),
            getOmnichainCheckpoint({ tag: "finalized" }),
            _crashRecoveryCheckpoint ?? ZERO_CHECKPOINT_STRING,
          ),
        ).blockTimestamp,
      ),
    };
  ...
  return {
    ...
    async startRealtime() {
      for (const perChainApp of perChainApps) {
        const { syncProgress, realtimeSync } = perChainSync.get(
          perChainApp.indexingBuild.chain,
        )!;
        ...
        if (isSyncEnd(syncProgress)) {
          ...
          for (const filter of getFilters(perChainApp)) {
            switch (filter.type) {
              case "log":
          ...
          app.common.logger.debug({
            service: "sync",
            msg: `Initialized '${perChainApp.indexingBuild.chain.chain.name}' realtime sync with ${initialChildAddresses.size} factory child addresses`,
          });
  ...
  return async (event: RealtimeSyncEvent): Promise<RealtimeSyncEvent> => {
    switch (event.type) {
      case "block": {
        ...
        app.common.logger.debug({
          service: "sync",
          msg: `Updated '${app.indexingBuild.chain.chain.name}' current block to ${hexToNumber(event.block.number)}`,
        });
      ...
      case "finalize": {
        ...
        app.common.logger.debug({
          service: "sync",
          msg: `Updated '${app.indexingBuild.chain.chain.name}' finalized block to ${hexToNumber(event.block.number)}`,
        });
...
export async function* getLocalEventGenerator(
  ...
  app.common.logger.debug({
    service: "sync",
    msg: `Initialized '${app.indexingBuild.chain.chain.name}' extract query for block range [${fromBlock}, ${toBlock}]`,
  });
  ...
  for await (const syncCursor of bufferAsyncGenerator(
    ...
    while (cursor < Math.min(syncCursor, toBlock)) {
      ...
      app.common.logger.debug({
        service: "sync",
        msg: `Extracted ${events.length} '${app.indexingBuild.chain.chain.name}' events for block range [${cursor}, ${queryCursor}]`,
      });
      ...
      if (cursor === toBlock) {
        ...
        const checkpoint = encodeCheckpoint({
          ...
          chainId: BigInt(app.indexingBuild.chain.chain.id),
...
export async function* getLocalSyncGenerator(
  ...
  const label = { chain: app.indexingBuild.chain.chain.name };
  ...
  if (
    ...
    app.common.logger.warn({
      service: "sync",
      msg: `Skipped '${app.indexingBuild.chain.chain.name}' historical sync because the start block is unfinalized`,
    });
  ...
  app.common.logger.debug({
    service: "sync",
    msg: `Initialized '${app.indexingBuild.chain.chain.name}' historical sync for block range [${totalInterval[0]}, ${totalInterval[1]}]`,
  });
  ...
  if (syncProgress.current !== undefined) {
    ...
    if (hexToNumber(syncProgress.current.number) === hexToNumber(last.number)) {
      app.common.logger.info({
        service: "sync",
        msg: `Skipped '${app.indexingBuild.chain.chain.name}' historical sync because all blocks are cached`,
      });
      return;
    } else {
      app.common.logger.info({
        service: "sync",
        msg: `Started '${app.indexingBuild.chain.chain.name}' historical sync with ${formatPercentage(
    ...
    app.common.logger.info({
      service: "historical",
      msg: `Started '${app.indexingBuild.chain.chain.name}' historical sync with 0% cached`,
    });
  }
  ...
  while (true) {
    ...
    app.common.logger.debug({
      service: "sync",
      msg: `Synced ${interval[1] - interval[0] + 1} '${app.indexingBuild.chain.chain.name}' blocks in range [${interval[0]}, ${interval[1]}]`,
    });
    ...
    if (synced === undefined) {
      ...
      app.common.logger.trace({
        service: "sync",
        msg: `Updated '${app.indexingBuild.chain.chain.name}' historical sync estimate to ${estimateRange} blocks`,
      });
    }
    ...
    if (isSyncEnd(syncProgress) || isSyncFinalized(syncProgress)) {
      app.common.logger.info({
        service: "sync",
        msg: `Completed '${app.indexingBuild.chain.chain.name}' historical sync`,
      });
      return;
    }
  }
...
export const getLocalSyncProgress = async (
  app: PerChainPonderApp,
  { intervalsCache }: { intervalsCache: HistoricalSync["intervalsCache"] },
  ...
  // TODO(kyle) do we need to request start and end?

  const diagnostics = await Promise.all(
    cached === undefined
      ? [
          app.indexingBuild.chain.rpc.request({ method: "eth_chainId" }),
  ...
  const finalized = Math.max(
    ...
    hexToNumber(diagnostics[1].number) -
      app.indexingBuild.chain.finalityBlockCount,
  );
  syncProgress.finalized = await _eth_getBlockByNumber(
    app.indexingBuild.chain.rpc,
  ...
  if (hexToNumber(diagnostics[0]) !== app.indexingBuild.chain.chain.id) {
    app.common.logger.warn({
      service: "sync",
      msg: `Remote chain ID (${diagnostics[0]}) does not match configured chain ID (${app.indexingBuild.chain.chain.id}) for chain "${app.indexingBuild.chain.chain.name}"`,
    });
  }
