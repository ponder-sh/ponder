...
export const perChainOnRealtimeSyncEvent = async (
  event: RealtimeSyncEvent,
  {
    common,
    chain,
  ...
  switch (event.type) {
    ...
    case "finalize": {
      ...
      const finalizedBlocks: Omit<
        Extract<RealtimeSyncEvent, { type: "block" }>,
        "type"
      >[] = [];
      for (let i = 0; i < unfinalizedBlocks.length; i++) {
        if (
          hexToNumber(unfinalizedBlocks[i]!.block.number) <=
          hexToNumber(event.block.number)
        ) {
          finalizedBlocks.push(unfinalizedBlocks[i]!);
          unfinalizedBlocks.splice(i, 1);
        }
      }
