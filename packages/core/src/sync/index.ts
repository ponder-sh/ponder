...
export const createSync = async (args: CreateSyncParameters): Promise<Sync> => {
  const localSyncContext = new Map<
    Network,
    {
      ...
      localEventData: BlockWithEventData[];
    }
  >();
  const status: Status = {};
  let isKilled = false;
  ...
  const eventQueue = createQueue({
    ...
    worker: async ({
      ...
      const { syncProgress, realtimeSync, localEventData } =
      ...
      switch (event.type) {
        ...
        case "block": {
          ...
          // TODO(kyle) make sure `block.transactions` is not saved to memory

          localEventData.push({
            block: event.block,
            filters: event.filters,
            logs: event.logs,
            factoryLogs: event.logs,
          ...
          if (to > from) {
            ...
            for (const network of args.networks) {
              ...
              for (const blockWithEventData of localEventData) {
        ...
        case "finalize": {
          ...
          const finalizedEventData = localEventData.filter(
            (led) =>
              hexToNumber(led.block.number) <= hexToNumber(event.block.number),
          );

          localSyncContext.get(network)!.localEventData = localEventData.filter(
            (led) =>
              hexToNumber(led.block.number) > hexToNumber(event.block.number),
          );
        ...
        case "reorg": {
          ...
          await args.syncStore.pruneRpcRequestsByBlock({
