...
// @ts-ignore
import type { CustomEventMap, HMRPayload, Update } from "vite";
// @ts-ignore
import type { HotContext } from "vite-node";
// @ts-ignore
import type { ViteNodeRunner } from "vite-node/client";
// @ts-ignore
import type { HMREmitter } from "vite-node/hmr";
// @ts-ignore
...
function sendMessageBuffer(runner: ViteNodeRunner, emitter: HMREmitter) {
  const maps = getCache(runner);
  maps.messageBuffer.forEach((msg) => emitter.emit("custom", msg));
  maps.messageBuffer.length = 0;
}

async function reload(runner: ViteNodeRunner, files: string[]) {
...
async function fetchUpdate(
  ...
  if (isSelfUpdate || qualifiedCallbacks.length > 0) {
    ...
    try {
      console.log("in fetchUpdate before reload", {
        acceptedPath,
        cached: runner.moduleCache.keys(),
      });
      [fetchedModule] = await reload(runner, [acceptedPath]);
      console.log("in fetchUpdate after reload", {
        acceptedPath,
        cached: runner.moduleCache.keys(),
      });
    } catch (e: any) {
...
export function createHotContext(
  ...
  const hot: HotContext = {
    ...
    invalidate() {
      ...
      console.log("in invalidate before reload", {
        cached: runner.moduleCache.keys(),
      });
