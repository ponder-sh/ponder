...
export const buildSchema = ({ schema }: { schema: Schema }) => {
  ...
  // Validate tables
  Object.entries(getTables(schema)).forEach(([tableName, { table, constraints }]) => {
    validateTableOrColumnName(tableName, "Table");

    // Validate the id column
    if (table.id === undefined)
      throw new Error(`Validation failed: Table '${tableName}' does not have an 'id' column.`);
    ...
    Object.entries(table).forEach(([columnName, column]) => {
      ...
      if (isOneColumn(column)) {
        const usedColumn = Object.entries(table).find(([c]) => c === column[" reference"]);

        if (usedColumn === undefined) {
          const otherColumns = Object.keys(table).filter((c) => c !== columnName);
          throw new Error(
      ...
      if (isManyColumn(column)) {
        ...
        if (usedTable === undefined) {
          const otherTables = Object.keys(getTables(schema)).filter((t) => t !== tableName);
        ...
        if (!isReferenceColumn(usedColumn[1])) {
          const foreignKeyColumnNames = usedTableColumns.filter(([, c]) => isReferenceColumn(c));
          throw new Error(
            `Validation failed. Relationship column '${tableName}.${columnName}' uses a column that is not foreign key column. Got '${
              column[" referenceTable"]
    ...
    for (const [name, index] of Object.entries(constraints)) {
      ...
      if (Array.isArray(column)) {
        if (column.length === 0)
          throw new Error(`Validation failed: Index '${name}' cannot be empty.`);

        if (column.length !== dedupe(column as string[]).length)
          throw new Error(`Validation failed: Index '${name}' cannot contain duplicate columns.`);
        ...
        if (table[column] === undefined)
          throw new Error(
            `Validation failed: Index '${name}' does not reference a valid column. Got '${column}', expected one of [${Object.entries(
              ...
              .filter(([_, column]) => !isOneColumn(column) && !isManyColumn(column))
...
const validateTableOrColumnName = (key: string, type: string) => {
  if (key === "") throw new Error(`Validation failed: ${type} name can't be an empty string.`);

  if (!/^[a-z|A-Z|0-9]+$/.test(key))
    throw new Error(`Validation failed: ${type} name '${key}' contains an invalid character.`);
};
