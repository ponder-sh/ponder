...
## Repository Shape
- This is a pnpm `11.7.0` workspace; install with `pnpm install --frozen-lockfile` to match CI. Monorepo development requires Node `>=22.13`; published packages still target Node `>=18.14` unless changed explicitly. CI currently uses Node `24.8` and a pinned Foundry nightly.
